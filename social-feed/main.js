...
function deliveryUrl(urls) {
  var url = firstUrl(urls);

  return (url && url.indexOf(exp.auth.api.host) === 0) ? url + '?_rt=' + exp.auth.restrictedToken : url;
}
...
function format(item) {
  ...
  if (data.src = youtubeUrl(item.videos)) {
    data.isYoutube = true;
  } else if (data.src = deliveryUrl(item.videos)) {
    data.isVideo = true;
  } else if (data.src = deliveryUrl(item.images)) {
    data.isImage = true;
  } else {
...
function load() {
  ...
  App.ApplicationRoute = Ember.Route.extend({
    ...
    setupController(controller, model) {
      ...
      controller.set('isLinkedIn', 'scala:feed:linkedIn' === feed.document.subtype);
      controller.set('isYammer', 'scala:feed:yammer' === feed.document.subtype);
